...
class RootCommand(Feature):
    ...
    async def jsk(self, ctx: ContextA):
        ...
        gaurav = discord.Embed(
                description="\n".join(summary),
                color=0x2f3136)
        gaurav.set_footer(text=f"Average websocket latency: {round(self.bot.latency * 1000, 2)}ms",
                             icon_url=self.bot.user.display_avatar.url)
        gaurav.set_thumbnail(url=self.bot.user.display_avatar.url)
        gaurav.set_author(name=ctx.author,icon_url=ctx.author.avatar.url
                if ctx.author.avatar else ctx.author.default_avatar.url)
        await ctx.send(embed=gaurav)
